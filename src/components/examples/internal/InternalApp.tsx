...
const statuses: Record<DeploymentStatus, string> = {
  offline: "text-neutral-500 bg-neutral-100/10",
  online: "text-green-400 bg-green-400/10",
  error: "text-rose-400 bg-rose-400/10",
};

const environments: Record<EnvironmentType, string> = {
  ...
  return (
    <>
      {/*
        ...
        ```
        <html class="h-full bg-neutral-900">
        <body class="h-full">
        ```
      */}
      <div>
          ...
          <main className="lg:pr-96">
            <header className="flex items-center justify-between border-b border-white/5 px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
              ...
              <Menu as="div" className="relative">
                ...
                >
                  <Menu.Items className="absolute right-0 z-10 mt-2.5 w-40 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-neutral-900/5 focus:outline-none">
                    <Menu.Item>
                      {({ active }) => (
                        <a
                          href="#"
              ...
              {activityItems.map((item) => (
                <li key={item.commit} className="px-4 py-4 sm:px-6 lg:px-8">
                  ...
                  <p className="mt-3 truncate text-sm text-neutral-500">
                    Pushed to{" "}
                    <span className="text-neutral-400">{item.projectName}</span>{" "}
                    (
                    <span className="font-mono text-neutral-400">
                      {item.commit}
                    </span>{" "}
                    on <span className="text-neutral-400">{item.branch}</span>)
                  </p>
                </li>
              ))}
            </ul>
